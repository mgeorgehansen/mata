...
private:
  ...
  static std::size_t nBytesPerTile(const GridDimensions2d tileSize) {
    return static_cast<std::size_t>(tileSize.nColumns * tileSize.nRows *
                                    N_COLOR_CHANNELS);
  }

  GridDimensions2d m_tileSize;
  GridDimensions2d m_dimensions;
  Texture m_texture;

public:
  Impl(const GridDimensions2d tileSize, const GridDimensions2d dimensions,
  ...
  GridDimensions2d tileSize() const noexcept { return m_tileSize; }
  ...
  //                           +--------+
  // ... etc.
  const bytes asLinearBytes() const noexcept {
    const auto textureBytes = m_texture.asBytes();
    auto linearBytes = bytes(textureBytes.size());
...
Tileset::Tileset(const GridDimensions2d tileSize,
...
GridDimensions2d Tileset::dimensions() const noexcept {
  return m_pImpl->dimensions();
}

GridDimensions2d Tileset::tileSize() const noexcept {
  return m_pImpl->tileSize();
}

const Texture &Tileset::texture() const noexcept { return m_pImpl->texture(); }

bytes Tileset::asLinearBytes() const noexcept {
